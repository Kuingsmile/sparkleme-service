...
func createServiceConfig(executablePath string) *service.Config {
	...
	return &service.Config{
		Name:         "SparkleMeService",
		DisplayName:  "SparkleMe Service",
		Description:  "SparkleMe 提权服务",
...
var serviceInitCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		keyDir := filepath.Join(userDataDir, "sparkleme", "keys")
		...
		if status == service.StatusRunning {
			...
			if err := s.Restart(); err != nil {
				log.Println("重启服务失败：", err)
				log.Println("请手动执行 'sparkleme-service service restart' 命令")
				return
			}
			log.Println("服务已成功重启")
		} else {
